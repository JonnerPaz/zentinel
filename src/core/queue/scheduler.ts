...
export interface SchedulerOptions {
  retentionDays?: number; // Días que se conservan los logs (ej. 30 días)
  checkIntervalMs?: number; // Frecuencia con la que revisa y purga (ej. 24 horas)
}
...
export class CleanupScheduler {
  ...
  /**
   * Ejecuta el proceso de purga de registros antiguos.
   */
  private async runCleanup(): Promise<void> {
    try {
      ...
      const deletedCount = await this.storage.deleteOlderThan(cutoffDate);
      if (deletedCount > 0) {
        console.log(
          `[Logger Scheduler] Se eliminaron ${deletedCount} registros anteriores a ${cutoffDate.toISOString()}`,
        );
      }
    } catch (error) {
